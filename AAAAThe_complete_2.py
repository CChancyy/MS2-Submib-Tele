...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_keyboard = [["I am using a machine!"], ["I have collected my laundry"], ["Leaderboard"], ["Contact Previous User"]]
    ## here is the code to add the user's info to the teleusers document 
    if db.collection('teleusers').where('id', '==', str(update.message.from_user.id)).get() == []:
        data = {'id':str(update.message.from_user.id), 'point':0, 'username': str(update.message.from_user.username)}
        db.collection('teleusers').add(data) # store the user's info in firebase 
...
async def echo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if 'previous' in update.message.text: 
        ...
        if db.collection('washersE').where('previous', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('washersE').where('previous', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('prepre')
                if len(target_chat_id) > 3:
                    ...
                    await context.bot.send_message(chat_id=update.effective_chat.id, text="Sorry, we cannot find the previous user, please find the residential teacher for assistance.")

        if db.collection('washersF').where('previous', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('washersF').where('previous', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('prepre')
                if len(target_chat_id) > 3:
                    message = update.message
        ...
        if db.collection('dryersE').where('previous', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('dryersE').where('previous', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('prepre')
                if len(target_chat_id) > 3:
                    message = update.message
        ...
        if db.collection('dryersF').where('previous', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('dryersF').where('previous', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('prepre')
                if len(target_chat_id) > 3:
                    message = update.message
        ...
        if db.collection('washersE').where('used_by', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('washersE').where('used_by', '==', str(update.message.from_user.id)).get()
            for post in query:
        ...
        if db.collection('washersF').where('used_by', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('washersF').where('used_by', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('previous')
                if len(target_chat_id) > 3:
                    message = update.message
        ...
        if db.collection('dryersE').where('used_by', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('dryersE').where('used_by', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('previous')
                if len(target_chat_id) > 3:
                    message = update.message
        ...
        if db.collection('dryersF').where('used_by', '==', str(update.message.from_user.id)).get() != []:
            query = db.collection('dryersF').where('used_by', '==', str(update.message.from_user.id)).get()
            for post in query:
                target_chat_id = post.get('previous')
                if len(target_chat_id) > 3:
                    message = update.message
    ...
    if "using" in update.message.text: ## select location
        ...
        await update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup(
                reply_keyboard, one_time_keyboard=True, resize_keyboard = True, input_field_placeholder="Command"),)
    '''
    if "Washer" in update.message.text or "Dryer" in update.message.text: ## select location
        reply_keyboard = [["I am using a machine!"], ["I have collected my laundry"], ["Leaderboard"], ["Contact Previous User"]]
        await update.message.reply_text(
            reply_markup=ReplyKeyboardMarkup(
                reply_keyboard, one_time_keyboard=True, resize_keyboard = True, input_field_placeholder="Command"),)
    ...
    if "E Washer 1" in update.message.text:
        ref2 = db.collection("washersE").document("washers1")
    ...
    if "E Washer 2" in update.message.text:
        ref2 = db.collection("washersE").document("washers2")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "E Washer 3" in update.message.text:
        ref2 = db.collection("washersE").document("washers3")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "E Washer 4" in update.message.text:
        ref2 = db.collection("washersE").document("washers4")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Washer 1" in update.message.text:
        ref2 = db.collection("washersF").document("washers1")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Washer 2" in update.message.text:
        ref2 = db.collection("washersF").document("washers2")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Washer 3" in update.message.text:
        ref2 = db.collection("washersF").document("washers3")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Washer 4" in update.message.text:
        ref2 = db.collection("washersF").document("washers4")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})

    if "E Dryer 1" in update.message.text:
        ref2 = db.collection("dryersE").document("dryer1")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "E Dryer 2" in update.message.text:
        ref2 = db.collection("dryersE").document("dryer2")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "E Dryer 3" in update.message.text:
        ref2 = db.collection("dryersE").document("dryer3")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "E Dryer 4" in update.message.text:
        ref2 = db.collection("dryersE").document("dryer4")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Dryer 1" in update.message.text:
        ref2 = db.collection("dryersF").document("dryer1")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Dryer 2" in update.message.text:
        ref2 = db.collection("dryersF").document("dryer2")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Dryer 3" in update.message.text:
        ref2 = db.collection("dryersF").document("dryer3")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})
    if "F Dryer 4" in update.message.text:
        ref2 = db.collection("dryersF").document("dryer4")
        ref2.update({ 'prepre': str(ref2.get(field_paths={'previous'}).to_dict().get("previous")) , 
                     'previous': str(ref2.get(field_paths={'used_by'}).to_dict().get("used_by")), 
                        "avail": False,"used_by":str(update.message.from_user.id), 'StartTime':now, 'collect':False})


    if "collected" in update.message.text: 
        ...
        while collect_time > ( finish_time + timedelta(seconds = 30)) :
            ...
            query1 = db.collection('teleusers').where('id', '==', str(update.message.from_user.id)).get()
            for post in query1:
                ref2 = db.collection("teleusers").document(str(post.id))
            ...
            if db.collection('washersE').where('used_by', '==', str(update.message.from_user.id)).get() != []:
                query2 = db.collection('washersE').where('used_by', '==', str(update.message.from_user.id)).get()
                for post2 in query2:
                    ref3 = db.collection("washersE").document(str(post2.id))
                    ref3.update({"collect": True})
                    #await update.message.reply_text("finish changing the status of collection to true")   
            if db.collection('washersF').where('used_by', '==', str(update.message.from_user.id)).get() != []:
                query2 = db.collection('washersF').where('used_by', '==', str(update.message.from_user.id)).get()
                for post2 in query2:
                    ref3 = db.collection("washersF").document(str(post2.id))
                    ref3.update({"collect": True})
                    #await update.message.reply_text("finish changing the status of collection to true")   
            if db.collection('dryersE').where('used_by', '==', str(update.message.from_user.id)).get() != []:
                query2 = db.collection('dryersE').where('used_by', '==', str(update.message.from_user.id)).get()
                for post2 in query2:
                    ref3 = db.collection("dryersE").document(str(post2.id))
                    ref3.update({"collect": True})
                    #await update.message.reply_text("finish changing the status of collection to true")      
            if db.collection('dryersF').where('used_by', '==', str(update.message.from_user.id)).get() != []:
                query2 = db.collection('dryersF').where('used_by', '==', str(update.message.from_user.id)).get()
                for post2 in query2:
                    ref3 = db.collection("dryersF").document(str(post2.id))
                    ref3.update({"collect": True})
                    #await update.message.reply_text("finish changing the status of collection to true")      

    if 'Leaderboard' in update.message.text: 
        # write code here
        query = db.collection('teleusers').order_by('point',direction=firestore.Query.DESCENDING).limit(5)
        results = query.get()
        a = 1
        message = ''
        for post in results:
